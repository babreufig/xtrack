...
``c_src/xt_bridge.cpp`` is compiled per flavor through ``ContextCpu.build_kernels``
(C++, linking ``xgtpsa``'s GTPSA core, the tpsa flavor also links the separate
``xt_bridge_knob.cpp`` TU), yielding cffi API-mode modules that export the track
functions. The built ``.so`` is cached on disk
...
def _bridge_cache_key(flavor: str) -> str:
    ...
    Content-addressed, so editing ``xt_bridge.cpp`` busts the cache by itself.
    There is no version constant to remember to bump. The xtrack rev covers the physics headers, which
